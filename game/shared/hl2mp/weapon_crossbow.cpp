...
void CWeaponCrossbow::FireBolt(void)
{
	...
	CBasePlayer* pPlayer = ToBasePlayer(GetOwner());  // Changed from pOwner to pPlayer for consistency
	if (!pPlayer)
		return;
	...
	CCrossbowBolt* pBolt = CCrossbowBolt::BoltCreate(vecSrc, angAiming, GetHL2MPWpnData().m_iPlayerDamage, pPlayer);
	...
	// Modified recoil with proper player pointer
	QAngle punchAngle(
		sv_crossbow_recoil_pitch.GetFloat(), // Pitch
		0,                                   // Yaw
		0                                    // Roll
	);
	pPlayer->ViewPunch(punchAngle);  // Fixed: Using pPlayer instead of undefined pOwner
...
void CWeaponCrossbow::ToggleZoom(void)
{
	...
	if (m_bInZoom)
	{
		pPlayer->SetFOV(this, 0, 0.2f); // Return to normal FOV
		m_bInZoom = false;
	}
	else
	{
		pPlayer->SetFOV(this, sv_crossbow_zoom_fov.GetInt(), 0.1f);
		m_bInZoom = true;
	}
#endif
}
