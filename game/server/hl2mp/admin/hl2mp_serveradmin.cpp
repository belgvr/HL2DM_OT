...
void CheckRTVThreshold(CBasePlayer* pPlayer)
{
	...
	{
		UTIL_PrintToAllClients(CHAT_LIME "Voting for next map has started...\n");
		g_votebegun = true;
	...
	else
	{
		if (!pPlayer->IsDisconnecting())
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "wants to rock the vote " CHAT_LIME "(%d/%d votes needed)\n", pPlayer->GetPlayerName(), g_votes, g_votesneeded));

		/*UTIL_PrintToAllClients(UTIL_VarArgs(
			CHAT_ADMIN "Rock the vote progress: " CHAT_LIME "%d/%d votes needed\n",
...
static void AdminSay(const CCommand& args)
{
	...
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "You do not have permission to use this command.\n");
		return;
	}
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN "Usage: " CHAT_ADMIN_HIGHLIGHT "sa say <message>\n");
		}
		return;
	}
	...
	{
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_LIME "(ADMIN) %s:" CHAT_WHITE " %s\n", pPlayer->GetPlayerName(), messageText.Get()));
	}
}
...
static void AdminCSay(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa csay <message>\n");
		}
		return;
	}
...
static void AdminChat(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa chat <message>\n");
		}
		return;
	}
	...
	if (isServerConsole)
	{
		formattedMessage = UTIL_VarArgs(CHAT_LIME "(Admin Chat) Console:" CHAT_WHITE " % s\n", messageText.Get());
	}
	else
	{
		formattedMessage = UTIL_VarArgs(CHAT_LIME "(Admin Chat) %s:" CHAT_WHITE " %s\n", pPlayer->GetPlayerName(), messageText.Get());
	}
...
static void AdminPSay(const CCommand& args)
{
	...
	{
		UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "You do not have permission to use this command.\n");
		return;
	}
	...
	if (args.ArgC() < 4)
	{
		...
		{
			UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa psay <name|#userID> <message>\n");
		}
		return;
	}
	...
	if (targetPlayerInput[0] == '#')
	{
		...
		if (userID > 0)
		{
			...
			if (!pTarget)
			{
				UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "No player found with that UserID.\n");
				return;
			}
		}
		else
		{
			UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "Invalid UserID provided.\n");
			return;
		}
	}
	else
	{
		...
		{
			UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "No players found matching that name.\n");
			return;
		}
		else if (matchingPlayers.Count() > 1)
		{
			UTIL_PrintToClient(pSender, CHAT_ADMIN_ERROR "Multiple players match that partial name:\n");
			for (int i = 0; i < matchingPlayers.Count(); i++)
			{
				UTIL_PrintToClient(pSender, UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s\n", matchingPlayers[i]->GetPlayerName()));
			}
			return;
		}
	...
	if (isServerConsole)
	{
		formattedMessage = UTIL_VarArgs(CHAT_LIME "[PRIVATE] Console:" CHAT_WHITE " %s\n", messageText.Get());
	}
	else
	{
		formattedMessage = UTIL_VarArgs(CHAT_LIME "[PRIVATE] % s:" CHAT_WHITE " % s\n", pSender->GetPlayerName(), messageText.Get());
	}
	...
	if (pTarget != pSender)
	{
		...
		{
			UTIL_PrintToClient(pSender, UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "[PRIVATE] %s: " CHAT_DM "%s\n", pSender->GetPlayerName(), messageText.Get()));
		}
	}
...
static void CreditsCommand(const CCommand& args)
{
	...
	if (!pPlayer && isServerConsole)
	{
		...
		Msg("Special thanks to all of the brazilian Oppressive Territory Community members for keeping this game alive and fun forever.\n\n");
	}
	else if (pPlayer && !isServerConsole)
	{
		...
		ClientPrint(pPlayer, HUD_PRINTCONSOLE, "DGB\n");
		ClientPrint(pPlayer, HUD_PRINTCONSOLE, "Special thanks to all the HL2DM players keeping the game alive!\n\n");
		ClientPrint(pPlayer, HUD_PRINTCONSOLE, "Special thanks to all of the brazilian Oppressive Territory Community members for keeping this game alive and fun forever.\n\n");
	}
}
...
static void ExecFileCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa exec <filename>\n");
		}
		return;
	}
	...
	{
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_LIGHT "Executing config file: " CHAT_ADMIN_HIGHLIGHT "%s\n", filename));
	}
}
...
static void NoClipPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa noclip <name|#userID>\n");
		}
		return;
	}
	...
	if (targetPlayerInput[0] == '@')
	{
		if (HL2MPAdmin()->FindSpecialTargetGroup(targetPlayerInput))
		{
			...
			if (Q_stricmp(targetPlayerInput, "@dead") == 0)
			{
				...
				{
					UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Can't target dead players.\n");
				}
				return;
			}
			else if (isServerConsole && Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				if (isServerConsole)
					...
					UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Can't use " CHAT_ADMIN_HIGHLIGHT "@me:" CHAT_ADMIN_ERROR " from the server console\n");
				return;
			}
			else if (isServerConsole && Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
			...
			if (targetPlayers.Count() == 0)
			{
				...
				{
					UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "No players found matching the target group.\n");
				}
				return;
			}
		}
		else
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Invalid special target specifier.\n");
			}
			return;
		}
	}
	else if (targetPlayerInput[0] == '#')
	{
		...
		if (userID > 0)
		{
			...
			if (!pTarget)
			{
				...
				{
					UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "No player found with that UserID.\n");
				}
				return;
			}
		}
		else
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Invalid UserID provided.\n");
			}
			return;
		}
	}
	else
	{
		...
		if (matchingPlayers.Count() == 0)
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "No players found matching that name.\n");
			}
			return;
		}
		else if (matchingPlayers.Count() > 1)
	...
	if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "toggled noclip for all players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", targetPlayerInput + 1));
		...
		else
		{
			...
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "toggled noclip for themself.\n", pAdmin->GetPlayerName()));
				logMessage = "themself";
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "toggled noclip for all players except themself.\n", pAdmin->GetPlayerName()));
				logMessage = "all players except themself";
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "toggled noclip for players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pAdmin->GetPlayerName(), targetPlayerInput + 1));
				Q_snprintf(logBuffer, sizeof(logBuffer), "players in group %s", targetPlayerInput + 1);
	...
	else if (pTarget)
	{
		if (!pTarget->IsAlive())
		{
			UTIL_PrintToClient(pAdmin, UTIL_VarArgs(CHAT_ADMIN_ERROR "This player is currently dead.\n", pTarget->GetPlayerName()));
			return;
		}
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "toggled noclip for " CHAT_ADMIN_HIGHLIGHT "%s\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "toggled noclip for " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Player not found.\n");
		}
	}
}
...
static void GotoPlayerCommand(const CCommand& args)
{
	...
	if (!pAdmin)
	{
		UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Command must be issued by a player.\n");
		return;
	}
	...
	if (pTarget)
	{
		if (!pTarget->IsAlive())
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "This player is currently dead.\n");
			return;
		}
		...
		if (pAdmin->IsAlive())
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "teleported to " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "You must be alive to teleport to a player.\n");
		}
	}
	else
	{
...
static void BringPlayerCommand(const CCommand& args)
{
	...
	if (pTarget)
	{
		if (pAdmin->IsAlive() && pAdmin->GetTeamNumber() != TEAM_SPECTATOR)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "teleported player " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "You must be alive to teleport a player.\n");
		}
	}
	else
	{
...
static void TeamPlayerCommand(const CCommand& args)
{
	...
	if (teamIndex < 1 || teamIndex > 3)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Invalid team index. Team index must be between 1 and 3.\n");
		}
		return;
	}
	...
	if (targetPlayerInput[0] == '@')
	{
		if (isServerConsole && Q_stricmp(targetPlayerInput, "@me") == 0)
		{
			if (isServerConsole)
				...
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Can't use @me from the server console\n");
			return;
		}
		else if (isServerConsole && Q_stricmp(targetPlayerInput, "@!me") == 0)
		{
	...
	if (pTarget)
	{
			...
			teamIndex != 1)
		{
			...
			{
				UTIL_PrintToClient(pAdmin, UTIL_VarArgs(CHAT_ADMIN "Player " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "is already on team " CHAT_ADMIN_HIGHLIGHT "%s.\n", pTarget->GetPlayerName(), teamName));
			}
			return;
		}
		...
		if (isServerConsole)
		{
			...
			sTeamMessage = UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "moved player " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "to team " CHAT_ADMIN_HIGHLIGHT "%s.\n", pTarget->GetPlayerName(), teamName);
		}
		else
		{
			sTeamMessage = UTIL_VarArgs(
				CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "moved player " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "to team " CHAT_ADMIN_HIGHLIGHT "%s.\n",
	...
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "moved players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "to team " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", targetPlayerInput + 1, teamName));
		}
		else
		{
			if (Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "moved themself to team " CHAT_ADMIN_HIGHLIGHT "%s.\n", pAdmin->GetPlayerName(), teamName));
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "moved all players except themself to team " CHAT_ADMIN_HIGHLIGHT "%s.\n", pAdmin->GetPlayerName(), teamName));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "moved players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "to team " CHAT_ADMIN_HIGHLIGHT "%s.\n", pAdmin->GetPlayerName(), targetPlayerInput + 1, teamName));
			}
		}
	}
...
static void UnMutePlayerCommand(const CCommand& args)
{
	...
	if (pTarget)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Cannot target this player (root admin privileges).\n");
			return;
		}
		...
		if (!pTarget->IsMuted())
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Player is not muted.\n");
			}
			return;
		}
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "has unmuted " CHAT_ADMIN_HIGHLIGHT "%s\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "has unmuted " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "unmuted all players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", targetPlayerInput + 1));
		}
		else
		{
			if (Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "unmuted themself.\n", pAdmin->GetPlayerName()));
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "unmuted all players except themself.\n", pAdmin->GetPlayerName()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "unmuted players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pAdmin->GetPlayerName(), targetPlayerInput + 1));  // Skip the '@'
			}
		}
	}
	else
...
static void MutePlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa mute <name|#userID>\n");
		}
		return;
	}
	...
	if (pTarget)
	{
		...
		if (pTarget->IsMuted())
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Player is already muted.\n");
			}
			return;
		}
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "has muted " CHAT_ADMIN_HIGHLIGHT "%s\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "has muted " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "muted all players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", targetPlayerInput + 1));
		}
		else
		{
			if (Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "muted themself.\n", pAdmin->GetPlayerName()));
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "muted all players except themself.\n", pAdmin->GetPlayerName()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "muted players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pAdmin->GetPlayerName(), targetPlayerInput + 1));  // Skip the '@'
			}
		}
	}
	else
...
static void UnGagPlayerCommand(const CCommand& args)
{
	...
	if (pTarget)
	{
		...
		if (!pTarget->IsGagged())
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Player is not gagged.\n");
			}
			return;
		}
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "has ungagged " CHAT_ADMIN_HIGHLIGHT "%s\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "has ungagged " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "ungagged all players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", targetPlayerInput + 1));
		}
		else
		{
			if (Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "ungagged themself.\n", pAdmin->GetPlayerName()));
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "ungagged all players except themself.\n", pAdmin->GetPlayerName()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "ungagged players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pAdmin->GetPlayerName(), targetPlayerInput + 1));  // Skip the '@'
			}
		}
	}
	else
...
static void GagPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa gag <name|#userID>\n");
		}
		return;
	}
	...
	if (pTarget)
	{
		...
		if (pTarget->IsGagged())
		{
			...
			{
				UTIL_PrintToClient(pAdmin, CHAT_ADMIN_ERROR "Player is already gagged.\n");
			}
			return;
		}
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Console " CHAT_ADMIN_LIGHT "has gagged " CHAT_ADMIN_HIGHLIGHT "%s\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_ADMIN_LIGHT "%s " CHAT_ADMIN "has gagged " CHAT_ADMIN_HIGHLIGHT "%s\n", pAdmin->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Console " CHAT_ADMIN "gagged all players in group " CHAT_ADMIN "%s.\n", targetPlayerInput + 1));
		}
		else
		{
			if (Q_stricmp(targetPlayerInput, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "gagged themself.\n", pAdmin->GetPlayerName()));
			}
			else if (Q_stricmp(targetPlayerInput, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "gagged all players except themself.\n", pAdmin->GetPlayerName()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "gagged players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pAdmin->GetPlayerName(), targetPlayerInput + 1));  // Skip the '@'
			}
		}
	}
	else
...
static void MapCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa map <mapname>\n");
		}
		return;
	}
	...
	if (HL2MPRules()->IsMapChangeOnGoing())
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "A map change is already in progress...\n");
		}
		return;
	}
	...
	if (exactMatchMap)
	{
		...
		if (isServerConsole)
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Console " CHAT_ADMIN_LIGHT "is changing the map to " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " in 5 seconds...\n", exactMatchMap));
		else
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_ADMIN_LIGHT "%s " CHAT_ADMIN "is changing the map to " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " in 5 seconds...\n", pPlayer->GetPlayerName(), exactMatchMap));
	...
	if (matchingMaps.Count() == 0)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_ERROR "No maps found matching " CHAT_ADMIN_HIGHLIGHT "%s\n", partialMapName));
		}
		return;
	}
	...
	else
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Multiple maps match the partial name:\n");
			for (int i = 0; i < matchingMaps.Count(); i++)
			{
...
static void RconCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa rcon <command> [arguments]\n"));
		}
		return;
	}
	...
	{
		UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "No " CHAT_ADMIN_HIGHLIGHT "\"sa rcon\" " CHAT_ADMIN "needed with commands starting with " CHAT_ADMIN_HIGHLIGHT "\"%s\"\n", commandName));
		return;
	}
	...
	if (pConVar && args.ArgC() == 3)
	{
		...
		if (fabs(currentValue - roundf(currentValue)) < 0.0001f)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "Cvar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " is currently set to " CHAT_ADMIN_HIGHLIGHT "%d.\n", commandName, intValue));
			}
		}
		else
		{
			...
			{
				UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "CVar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " is currently set to " CHAT_ADMIN_HIGHLIGHT "%f.\n", commandName, currentValue));
			}
		}
		return;
	}
	...
	{
		UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "Rcon command issued: " CHAT_ADMIN_HIGHLIGHT "%s\n", rconCommand.Get()));
	}
}
...
static void CVarCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa cvar <cvarname> [newvalue]\n"));
		}
		return;
	}
	...
	if (!pConVar)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_ERROR "Cvar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN_ERROR " not found.\n", cvarName));
		}
		return;
	}
	...
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "You do not have permission to change cheat protected cvars.\n");
		return;
	}
	...
	if (args.ArgC() == 3)
	{
		...
		if (Q_stricmp(cvarName, "sv_password") == 0)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Can't print the password. Use the server console directly.\n");
			}
			return;
		}
	...
	{
		pConVar->Revert();
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Cvar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " reset to default value.\n", cvarName));
	...
	if (Q_stricmp(cvarName, "sv_password") == 0)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "You do not have permission to add or change the server password.\n");
			return;
		}
		...
		if (Q_stricmp(pConVar->GetString(), newValue) == 0)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "Cvar " CHAT_ADMIN_HIGHLIGHT "sv_password" CHAT_ADMIN " is already set to " CHAT_ADMIN_HIGHLIGHT "\"%s\".\n", newValue));
			}
			return;
		}
	...
	if (fabs(f_currentValue - roundf(f_currentValue)) < epsilon)
	{
		...
		if (fabs(f_inputValue - roundf(f_inputValue)) < epsilon)
		{
			...
			if (int_currentValue == int_inputValue)
			{
				...
				{
					UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "Cvar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " is already set to " CHAT_ADMIN_HIGHLIGHT "%d.\n", cvarName, int_currentValue));
				}
				return;
			}
		}
	...
	if (f_currentValue == f_inputValue)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "Cvar " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " is already set to " CHAT_ADMIN_HIGHLIGHT "%f.\n", cvarName, f_currentValue));
		}
		return;
	}
	...
	{
		Msg("CVar %s set to %s.\n", cvarName, newValue);
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "changed cvar value " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " to " CHAT_ADMIN_HIGHLIGHT "%s.\n", cvarName, newValue));
	}
	else
	{
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "changed cvar value " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN " to " CHAT_ADMIN_HIGHLIGHT "%s.\n", pPlayer->GetPlayerName(), cvarName, newValue));
	}
}
...
static void SlapPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN "Usage: " CHAT_ADMIN_HIGHLIGHT "sa slap <name|#userid> [amount]\n");
		}
		return;
	}
	...
	if (partialName[0] == '@')
	{
		if (Q_stricmp(partialName, "@dead") == 0)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Can't target dead players.\n");
			}
			return;
		}
		else if (isServerConsole && Q_stricmp(partialName, "@me") == 0)
		{
			if (isServerConsole)
				...
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Can't use @me from the server console\n");
			return;
		}
		else if (isServerConsole && Q_stricmp(partialName, "@!me") == 0)
		{
		...
		if (HL2MPAdmin()->FindSpecialTargetGroup(partialName))
		{
			...
			if (targetPlayers.Count() == 0)
			{
				...
				{
					UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "No players found matching the target group.\n");
				}
				return;
			}
		}
		else
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid special target specifier.\n");
			}
			return;
		}
	}
	else if (partialName[0] == '#')
	{
		...
		if (userID > 0)
		{
			...
			if (!pTarget)
			{
				...
				{
					UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "No player found with that UserID.\n");
				}
				return;
			}
		}
		else
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid UserID provided.\n");
			}
			return;
		}
	}
	else
	{
		...
		if (matchingPlayers.Count() == 0)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "No players found matching that name.\n");
			}
			return;
		}
		else if (matchingPlayers.Count() > 1)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Multiple players match that partial name:\n");
				for (int i = 0; i < matchingPlayers.Count(); i++)
				{
					UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s\n", matchingPlayers[i]->GetPlayerName()));
				}
			}
			return;
		}
	...
	if (pTarget)
	{
		if (!pTarget->IsAlive())
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "This player is currently dead.\n");
			}
			return;
		}
		...
		if (isServerConsole)
		{
			if (slapDamage > 0)
			{
				...
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slapped " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", pTarget->GetPlayerName(), slapDamage));
			}
			else
			{
				Msg("Slapped %s\n", pTarget->GetPlayerName());
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slapped " CHAT_ADMIN_HIGHLIGHT "%s.\n", pTarget->GetPlayerName()));
			}
		}
		else
		{
			if (slapDamage > 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "slapped " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", pPlayer->GetPlayerName(), pTarget->GetPlayerName(), slapDamage));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "slapped " CHAT_ADMIN_HIGHLIGHT "%s.\n", pPlayer->GetPlayerName(), pTarget->GetPlayerName()));
			}
		}
	}
	else if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			if (slapDamage > 0)
			{
				...
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slapped all players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", partialName + 1, slapDamage));
			}
			else
			{
				Msg("Slapped all players in group %s.\n", partialName + 1);
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slapped all players in group " CHAT_ADMIN_HIGHLIGHT "%s.\n", partialName + 1));
			}
		}
		else
		{
			if (slapDamage > 0)
			{
				if (Q_stricmp(partialName, "@me") == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped themself for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", pPlayer->GetPlayerName(), slapDamage));
				}
				else if (Q_stricmp(partialName, "@!me") == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped all players except themself for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", pPlayer->GetPlayerName(), slapDamage));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped all players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d damage.\n", pPlayer->GetPlayerName(), partialName + 1, slapDamage));
				}
			}
			else
			{
				if (Q_stricmp(partialName, "@me") == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped themself.\n", pPlayer->GetPlayerName()));
				}
				else if (Q_stricmp(partialName, "@!me") == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped all players except themself.\n", pPlayer->GetPlayerName()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slapped all players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pPlayer->GetPlayerName(), partialName + 1));
				}
			}
		}
	}
	else
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Player not found.\n");
		}
	}
}
...
static void SlayPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa slay <name|#userid>\n");
		}
		return;
	}
	...
	if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slew players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", partialName + 1));
		}
		else
		{
			if (Q_stricmp(partialName, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slew themself.\n", pPlayer->GetPlayerName()));
				logDetails = "themself";
			}
			else if (Q_stricmp(partialName, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slew all players except themself.\n", pPlayer->GetPlayerName()));
				logDetails = "all players but themself";
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "slew players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pPlayer->GetPlayerName(), partialName + 1));
	...
	else if (pTarget)
	{
		if (!pTarget->IsAlive())
		{
			...
			{
				UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN_ERROR "Can't target dead players.\n", pTarget->GetPlayerName()));
			}
			return;
		}
		...
		{
			Msg("Slain %s\n", pTarget->GetPlayerName());
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "slew " CHAT_ADMIN_HIGHLIGHT "%s.\n", pTarget->GetPlayerName()));
		}
		else
		{
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "slew " CHAT_ADMIN_HIGHLIGHT "%s.\n", pPlayer->GetPlayerName(), pTarget->GetPlayerName()));
		}
	}
	else
	{
...
static void KickPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 3)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa kick <name|#userid> [reason]\n");
		}
		return;
	}
	...
	if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "kicked players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", partialName + 1));
		}
		else
		{
			if (Q_stricmp(partialName, "@me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "kicked themself from the server.\n", pPlayer->GetPlayerName()));
			}
			else if (Q_stricmp(partialName, "@!me") == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "kicked all players except themself.\n", pPlayer->GetPlayerName()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "kicked players in group " CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN ".\n", pPlayer->GetPlayerName(), partialName + 1));
			}
		}
	}
	else if (pTarget)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Cannot target this player (root admin privileges).\n");
			return;
		}
		...
		if (isServerConsole)
		{
			...
			if (reason.Length() > 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "kicked " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_SPEC "(%s)\n",
					pTarget->GetPlayerName(), reason.Get()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "kicked " CHAT_ADMIN_HIGHLIGHT "%s\n",
					pTarget->GetPlayerName()));
			}
		}
		else
		{
			if (reason.Length() > 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "kicked " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_SPEC "(%s)\n",
					pPlayer->GetPlayerName(), pTarget->GetPlayerName(), reason.Get()));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "kicked " CHAT_ADMIN_HIGHLIGHT "%s\n",
					pPlayer->GetPlayerName(), pTarget->GetPlayerName()));
			}
		}
	}
...
static void BanPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 4)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa ban <name|#userid> <time> [reason]\n");
		}
		return;
	}
	...
	if (!isValidTime)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid ban time provided.\n");
		}
		return;
	}
	...
	if (targetPlayers.Count() > 0)
	{
		...
		if (isServerConsole)
		{
			...
			if (banTime == 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "banned players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently.\n", partialName + 1));
			}
			else
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "banned players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n", partialName + 1, banTime, banTime > 1 ? "s" : ""));
			}
		}
		else
		{
			if (Q_stricmp(partialName, "@me") == 0)
			{
				if (banTime == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned themself permanently.\n", pPlayer->GetPlayerName()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned themself for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n", pPlayer->GetPlayerName(), banTime, banTime > 1 ? "s" : ""));
				}
			}
			else if (Q_stricmp(partialName, "@!me") == 0)
			{
				if (banTime == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned all players except themself permanently.\n", pPlayer->GetPlayerName()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned all players except themself for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n", pPlayer->GetPlayerName(), banTime, banTime > 1 ? "s" : ""));
				}
			}
			else
			{
				if (banTime == 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently.\n", pPlayer->GetPlayerName(), partialName + 1));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned players in group " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n", pPlayer->GetPlayerName(), partialName + 1, banTime, banTime > 1 ? "s" : ""));
				}
			}
		}
	}
	else if (pTarget)
	{
		...
		if (isServerConsole)
		{
			...
			if (banTime == 0)
			{
				...
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(
						CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently " CHAT_SPEC "(%s)\n",
						pTarget->GetPlayerName(), reason.Get()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(
						CHAT_ADMIN_HIGHLIGHT  "Console " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently.\n",
						pTarget->GetPlayerName()));
				}
			}
			else
			{
				...
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(
						CHAT_ADMIN_HIGHLIGHT "Console " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s. " CHAT_SPEC "(%s)\n",
						pTarget->GetPlayerName(), banTime, banTime > 1 ? "s" : "", reason.Get()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(
						CHAT_ADMIN_HIGHLIGHT  "Console " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d " "minute%s.\n",
		...
		else
		{
			if (banTime == 0)
			{
				if (reason.Length() > 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently " CHAT_SPEC "(%s).\n",
						pPlayer->GetPlayerName(), pTarget->GetPlayerName(), reason.Get()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently.\n",
						pPlayer->GetPlayerName(), pTarget->GetPlayerName()));
				}
			}
			else
			{
				if (reason.Length() > 0)
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s " CHAT_SPEC "(%s).\n",
						pPlayer->GetPlayerName(), pTarget->GetPlayerName(), banTime, banTime > 1 ? "s" : "", reason.Get()));
				}
				else
				{
					UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "Admin " CHAT_LIME "%s " CHAT_ADMIN "banned " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n",
...
static void AddBanCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 4)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa addban <time> <SteamID3> [reason]\n");
		}
		return;
	}
	...
	if (!closingBracket || idPart >= closingBracket)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid SteamID format. Missing closing bracket.\n");
		}
		return;
	}
	...
	for (const char* c = idPart; c < closingBracket; ++c)
	{
		if (!isdigit(*c))
		{
			...
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid SteamID format.\n" CHAT_ADMIN "The part after" CHAT_ADMIN_HIGHLIGHT " [U:1:" CHAT_ADMIN " must contain only numbers.\n");
			}
			return;
		}
	}
	...
	if (IsSteamIDAdmin(steamID))
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "This player is an admin and cannot be banned.\n");
		}
		return;
	}
	...
	if (file)
	{
		...
		while (filesystem->ReadLine(buffer, bufferSize, file))
		{
			if (Q_stristr(buffer, steamID))
			{
				...
				{
					UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "SteamID is already banned.\n");
				}
				filesystem->Close(file);
				return;
			}
	...
	else
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Failed to read the ban list.\n");
		}
		return;
	}
	...
	if (Q_strncmp(steamID, "[U:", 3) != 0 || Q_strlen(steamID) < 6)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid SteamID format.\n" CHAT_ADMIN "SteamID must start with " CHAT_ADMIN_HIGHLIGHT "[U:1: \n");
		}
		return;
	}
	...
	else
	{
		if (banTime == 0)
		{
			...
			if (Q_strlen(reason) > 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "SteamID " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently banned by " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN_HIGHLIGHT "(Reason: %s)\n",
					steamID,
					pPlayer->GetPlayerName(),
					reason));
			}
			...
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "SteamID " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "permanently banned by " CHAT_ADMIN_HIGHLIGHT "%s\n",
					steamID,
					pPlayer->GetPlayerName()));
			}
		}
		else
		{
			...
			if (Q_strlen(reason) > 0)
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "SteamID " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "banned by " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s. " CHAT_SPEC "(Reason: %s)\n",
					steamID,
					pPlayer->GetPlayerName(),
					banTime,
					banTime > 1 ? "s" : "",
			...
			{
				UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN "SteamID " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "banned by " CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "for " CHAT_ADMIN_HIGHLIGHT "%d minute%s.\n",
					steamID,
					pPlayer->GetPlayerName(),
					banTime,
					banTime > 1 ? "s" : ""));
...
static void UnbanPlayerCommand(const CCommand& args)
{
	...
	if (args.ArgC() < 7)
	{
		...
		{
			UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Usage: " CHAT_ADMIN_HIGHLIGHT "sa unban <SteamID3>\n");
		}
		return;
	}
	...
	if (file)
	{
		...
		if (steamIDFound)
		{
			...
			{
				UTIL_PrintToClient(pPlayer, UTIL_VarArgs(CHAT_ADMIN "SteamID " CHAT_ADMIN_LIGHT "%s " CHAT_ADMIN "has been " CHAT_ADMIN_HIGHLIGHT "unbanned.\n", steamID));
			}
...
static void AdminCommand(const CCommand& args)
{
	if (!g_bAdminSystem)
	{
		...
		else
		{
			...
			if (pPlayer)
			{
				UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Admin system disabled by the -noadmin launch command\n");
				return;
			}
		}
		return;
...
void NominateMapCommand(const CCommand& args)
{
	...
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "The maximum of " CHAT_ADMIN_HIGHLIGHT "5 " CHAT_ADMIN_ERROR "map nominations has already been reached.\n");
		return;
	}
	...
	if (!isValidMap)
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Invalid map selection. Please nominate a map from the cycle list.\n");
		return;
	}

	if (g_alreadyNominatedMaps.Find(selectedMap) != g_alreadyNominatedMaps.InvalidIndex())
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "This map has already been nominated by another player.\n");
		return;
	}
	...
	if (playerIndex == g_nominatedMaps.InvalidIndex())
	{
		...
		g_nominationCount++;
		UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s " CHAT_ADMIN "has nominated map " CHAT_ADMIN_HIGHLIGHT "%s\n", pPlayer->GetPlayerName(), selectedMap));
	}
	else
	{
		...
		if (previouslyNominatedMap != selectedMap)
		{
			...
			UTIL_PrintToAllClients(UTIL_VarArgs(CHAT_ADMIN_HIGHLIGHT "%s" CHAT_ADMIN "'s map nomination has been updated to " CHAT_ADMIN_HIGHLIGHT "%s\n", pPlayer->GetPlayerName(), selectedMap));
		}
		else
		{
...
void RtvCommand(const CCommand& args)
{
	...
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "Rock the vote is not available yet.\n");
		return;
	}
	else if (g_votehasended)
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "A map change is in progress...\n");
		return;
	}
	else if (g_votebegun)
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "A map vote is already in progress!\n");
		return;
	}
	else if (pPlayer->HasPlayerRTV())
	{
		UTIL_PrintToClient(pPlayer, CHAT_ADMIN_ERROR "You have already rocked the vote!\n");
		return;
	}
...
void CHL2MP_Admin::CheckChatText(char* p, int bufsize)
{
	...
	if (p[0] == '!' || p[0] == '/')
	{
		...
		else if (Q_strncmp(p, "!help", 5) == 0 || Q_strncmp(p, "/help", 5) == 0)
		{
			...
			{
				engine->ClientCommand(pPlayer->edict(), consoleCmd);
				UTIL_PrintToClient(pPlayer, CHAT_LIME "Check your " CHAT_ADMIN_HIGHLIGHT "console " CHAT_LIME "for output.\n");
			}
			return;
		}
